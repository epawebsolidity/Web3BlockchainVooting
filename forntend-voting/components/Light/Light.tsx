"use client";

import { useLight } from "@/context/LightContext";

export default function LightToggle() {
    const { on, setOn } = useLight();
    return (
        <div className="fixed bottom-20 right-4 z-50 ">
            <button
                onClick={() => setOn(!on)}
                className={`text-white border-2 ${on ? "border-black" : "border-white"} rounded-full`}
            >
                <div className={`flex items-center justify-center rounded-full p-4 ${on ? "bg-yellow-200" : "bg-black"}`}>
                    {on ? (
                        <svg className="w-[20px] h-[20px] text-black dark:text-black" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 5V3m0 18v-2M7.05 7.05 5.636 5.636m12.728 12.728L16.95 16.95M5 12H3m18 0h-2M7.05 16.95l-1.414 1.414M18.364 5.636 16.95 7.05M16 12a4 4 0 1 1-8 0 4 4 0 0 1 8 0Z" />
                        </svg>
                    ) : (
                            <svg className="w-[20px] h-[20px] text-white dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 21a9 9 0 0 1-.5-17.986V3c-.354.966-.5 1.911-.5 3a9 9 0 0 0 9 9c.239 0 .254.018.488 0A9.004 9.004 0 0 1 12 21Z" />
                            </svg>
                    )}
                </div>
            </button>
        </div>
    );
}
